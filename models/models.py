...
class PrecModel:
    ...
    def __init__(self, data, split_year, test_size, name):
        ...
        self.X_training = training_data.drop(['year', 'prec_GPCP_roll'], axis=1)
        self.y_training = training_data[["prec_GPCP_roll"]]

        # Split training and validation sets, 75% and 25% in the article
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(
            self.X_training, self.y_training, test_size=test_size, random_state=42)
    ...
    def _objective(self, trial):
        params = {
            ...
            "min_child_weight": trial.suggest_float("min_child_weight", 1e-3, 0.7, log=True),
        ...
        n_scores = cross_val_score(gbm, self.X_train, self.y_train,
                                   ...
                                   fit_params={'eval_metric': 'l2', 'eval_set': (self.X_val, self.y_val)})
    ...
    def fit(self):
        ...
        try:
            # Check whether a trained model exists and use that instead of retraining
            self.bst = lgb.Booster(model_file='models/lgb_{}_model.txt'.format(self.name))
    ...
    def calc_variance(self):
        """ Calculate the rolling variance, every 3 months, of the prediction error to measure uncertainty. """
        ...
        prec_lgb = pd.concat([preds_X_train_denorm, preds_X_val_denorm, preds_X_test_denorm], axis=0)
        prec_lgb.sort_index(inplace=True)

        # Calculate the error between observed data and predictions over the entire dataset
    ...
    def train_test_error(self):
        ...
        print("Training error: {}".format(mean_squared_error(preds_train, self.y_train)))
        print("Test error: {}".format(mean_squared_error(preds_test, self.y_test)))
    ...
    def print_errors(self):
        ...
        error_df = pd.DataFrame({'year': self.test_data['year'],
        ...
        for year in [2018, 2019]:
            for month in range(1, 13):
                ...
                rmse = np.sqrt(mean_squared_error(qry['prediction'], qry['reference']))
...
class UncModel(PrecModel):
    ...
    def save_data(self):
        ...
        unc_lgb = pd.concat([preds_X_train_denorm, preds_X_val_denorm, preds_X_test_denorm], axis=0)
